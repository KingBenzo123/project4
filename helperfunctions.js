...
export class vec4 extends Array {
    ...
    /**
     * Scales a four-dimensional vector (vec4) by a given factor.
     *
     * The purpose of scaling a vector is to stretch or shrink it. When you scale a vector,
     * each of its components (x, y, z, w) is multiplied by the same number (the scaling factor).
     * As a result:
     * - If the factor is greater than 1, the vector's magnitude increases (it gets stretched).
     * - If the factor is between 0 and 1, the vector's magnitude decreases (it gets shrunk).
     * - If the factor is 1, the vector remains unchanged.
     * - If the factor is 0, the resulting vector becomes the zero vector.
     * - Negative factors would reverse the vector's direction and scale its magnitude.
     *
     * @param {number} factor - The scaling factor.
     * @returns {vec4} A new vector where each component is multiplied by the given factor.
     */
    scale(factor) {
        return new vec4(this[0] * factor, this[1] * factor, this[2] * factor, this[3] * factor);
    }
}
//----------------------------------------------------------------------------
export class mat4 {
